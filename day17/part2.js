...
let findValidVelocities = (initialPosition, acceleration, target) => {
  // guess and check the max Y we should use.
  let validVelocities = []

  for (var y = target.y.min; y < 200; y++) {
...
let validVelocities = findValidVelocities(initialPosition, acceleration, target)

console.log(`Answer: ${validVelocities.length}`)
